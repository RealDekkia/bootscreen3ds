...
var write = function (x, y, text, color = 'gray') {
    while (text != '') {
        ...
        $('#topscreen').drawText({
            ...
            x: x + 2,
            y: y,
            fontSize: curDimensions.lh,
            fontFamily: 'PerfectDOSVGA437Win',
            align: 'left',
            text: letter
        });

        /* Remove the written character from the string, and if itn't empty, continue recursive */
        text = text.substr(1);
        x = x + curDimensions.lh / 2;
    }
}
...
var curDimensions = {
    ...
    /*Screen heigth*/
    h: 240,
    /*Device (switch or 3ds)*/
    d: '3ds',
    /*line-height*/
    lh: 16,
    /*left padding of top 2 lines*/
    tp: 24
};
var resizeImage = function (device) {
    curDimensions.d = device;
    var imgSrc;
    var imgW;
    var imgH;
    var imgSX;
    ...
    switch (device) {
        case '3ds':
            ...
            curDimensions.h = 240;
            curDimensions.lh = 16;
            imgSrc = 'images/symbols.png';
            imgW = 21;
            imgH = 29;
            imgSX = 40;
            imgSY = 10;
            curDimensions.tp = 24;
            $('#topscreen').width(curDimensions.w);
            $('#topscreen').height(curDimensions.h);
            break;
        case 'switch':
            curDimensions.w = 1280;
            curDimensions.h = 720;
            curDimensions.lh = 32;
            imgSrc = 'images/symbolsNX.png';
            imgW = 43;
            imgH = 58;
            imgSX = 0;
            imgSY = 336;
            curDimensions.tp = 48;
            $('#topscreen').width(curDimensions.w / 2);
            $('#topscreen').height(curDimensions.h / 2);
            break;
    }

    $('#topscreen').attr('width', curDimensions.w);
    $('#topscreen').attr('height', curDimensions.h);

    $('#topscreen').clearCanvas().drawRect({
        fillStyle: 'black',
        x: 0,
        y: 0,
        width: curDimensions.w,
        height: curDimensions.h
    }).drawImage({
        source: imgSrc,
        x: 1,
        y: curDimensions.lh,
...
function redraw() {
    ...
    switch (model) {
        case '3DS':
            resizeImage('3ds');
            write(0, curDimensions.lh * 5, 'Nintendo 3DS CTR-001(' + region + ')');
            processor = 2;
            sd += ' SD'
            break;
        case '3DSXL':
            resizeImage('3ds');
            if (region == 'JPN')
                write(0, curDimensions.lh * 5, 'Nintendo 3DS LL SPR-001(' + region + ')');
            else
                write(0, curDimensions.lh * 5, 'Nintendo 3DS XL SPR-001(' + region + ')');
            processor = 2;
            sd += ' SD'
            break;
        case '2DS':
            resizeImage('3ds');
            write(0, curDimensions.lh * 5, 'Nintendo 2DS FTR-001(' + region + ')');
            processor = 2;
            sd += ' SD'
            break;
        case 'n2DSXL':
            resizeImage('3ds');
            if (region == 'JPN')
                write(0, curDimensions.lh * 5, 'New Nintendo 2DS LL JAN-001(' + region + ')');
            else
                write(0, curDimensions.lh * 5, 'New Nintendo 2DS XL JAN-001(' + region + ')');
            processor = 4;
            sd += ' microSD'
            break;
        case 'n3DS':
            resizeImage('3ds');
            write(0, curDimensions.lh * 5, 'New Nintendo 3DS KTR-001(' + region + ')');
            processor = 4;
            sd += ' microSD'
            break;
        case 'n3DSXL':
            resizeImage('3ds');
            if (region == 'JPN')
                write(0, curDimensions.lh * 5, 'New Nintendo 3DS LL RED-001(' + region + ')');
            else
                write(0, curDimensions.lh * 5, 'New Nintendo 3DS XL RED-001(' + region + ')');
            processor = 4;
            sd += ' microSD'
            break;
        case 'neonSwitch':
            resizeImage('switch');
            write(0, curDimensions.lh * 5, 'Nintendo Switch HAC-001(' + region + ')');
            processor = 64;
            sd += ' microSD'
            break;
    }
    switch (type) {
        ...
        case 'reiNX2018':
            ...
            line2 = 'Copyright(C) 2018, Reisyukaku';
            break;
    ...
    write(curDimensions.tp, curDimensions.lh * 1, line1);
    write(curDimensions.tp, curDimensions.lh * 2, line2);

    switch (processor) {
        case 2:
            write(0, curDimensions.lh * 7, 'Main Processor       : Dual-core ARM11 MPCore');
            write(0, curDimensions.lh * 8, 'Memory Testing       : 131072K OK');
            break;
        case 4:
            write(0, curDimensions.lh * 7, 'Main Processor       : Quad-core ARM11 MPCore');
            write(0, curDimensions.lh * 8, 'Memory Testing       : 262144K OK');
            break;
        case 64:
            write(0, curDimensions.lh * 7, 'Main Processor       : NVIDIA Tegra X1');
    ...
    var bottomText1Y = 13;
    var bottomText2Y = 14;
    if (curDimensions.d == 'switch') {
        var bottomText1Y = 20.5;
        var bottomText2Y = 21.5;
    }

    if (boot_bool && !aux_bool)
        write(0, curDimensions.lh * bottomText2Y, boot_text);
    else if (boot_bool)
        write(0, curDimensions.lh * bottomText1Y, boot_text);

    if (aux_bool)
        write(0, curDimensions.lh * bottomText2Y, aux_text);

    if ($topscreen.width() == curDimensions.w * 2) {
        $topscreen.drawImage({
...
$('input[name=boottool]', "#settings").keyup(function () {
    $("#settings input").trigger('change');
});
$('input[name=auxtool]', "#settings").keyup(function () {
    $("#settings input").trigger('change');
});
...
$('#downloadPNG').click(function () {
    if (!$(this).hasClass('disabled')) {
        var filename = ($('#topscreen').width() == curDimensions.w) ? 'splash.png' : 'imagedisplay.png';
...
$('#downloadBIN').click(function () {
    if (!$(this).hasClass('disabled')) {
        var filename = ($('#topscreen').width() == curDimensions.w) ? 'splash.bin' : 'menuhax_imagedisplay.bin';
